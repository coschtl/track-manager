...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 8) {
            db.execSQL("ALTER TABLE " + TrackDbAdapter.DB.DATABASE_TABLE + " ADD " + TrackDbAdapter.DB.COL_AVG_PULSE + " integer default 0");
        }
        if (oldVersion < 9) {
            db.execSQL("ALTER TABLE " + TrackDbAdapter.DB.DATABASE_TABLE + " ADD " + TrackDbAdapter.DB.COL_MAX_PULSE + " integer default 0");
        }
