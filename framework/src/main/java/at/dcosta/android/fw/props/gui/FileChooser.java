...
public class FileChooser extends IconListActivity {
    ...
    private void fill() {
        ...
        try {
            ...
            for (File ff : dirs) {
                if (ff.isDirectory() && !IGNORE_DIRS.contains(ff.getAbsolutePath())) {
                    folders.add(new Option(i++, ff.getName(), "Folder", ff.getAbsolutePath(), R.mipmap.folder));
                } else {
                    files.add(new Option(i++, ff.getName(), "File", ff.getAbsolutePath(), R.mipmap.file));
                }
            }
        } catch (Exception e) {
            // ignore
        ...
        if (!"".equalsIgnoreCase(currentDir.getName())) {
            if (!isFileMode()) {
                currentList.add(new Option(-1, CURRENT_DIR, getString(R.string.actual_directory), currentDir.getAbsolutePath(), R.mipmap.folder));
            }
            currentList.add(new Option(-1, PARENT_DIR, getString(R.string.parent_directory), currentDir.getParent(), R.mipmap.folder));
        }
