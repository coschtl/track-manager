...
public class CalendarAdapter extends BaseAdapter {
	...
	private final Month<TrackDescription> month;
	private final Map<Date, List<TrackDescription>> tracks;
	private final GridView.LayoutParams layoutParams;
	private final int COLOR_DEFAULT = Color.parseColor("#FE2E2E");

	public CalendarAdapter(Context c, Month<TrackDescription> month, Map<Date, List<TrackDescription>> tracks) {
		context = c;
		this.month = month;
		this.tracks = tracks;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (dayDate != null && tracks.containsKey(dayDate)) {
			if (!day.hasContent()) {
				for (TrackDescription descr : tracks.get(dayDate)) {
		...
		if (day.hasContent()) {
			List<TrackDescription> content = day.getContent();
			int color = 0;
			for (TrackDescription descr : content) {
